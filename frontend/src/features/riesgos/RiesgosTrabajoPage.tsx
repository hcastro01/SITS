...
export function RiesgosTrabajoPage() {
  ...
  return <section className="module-list-page riesgos-trabajo-page">
    <header className="module-page-hero"><div><p className="eyebrow">Trabajo Social · Departamento Médico</p><h2>Riesgos de trabajo</h2><p>Registro y seguimiento operativo de Riesgos de trabajo.</p></div>{canCreate && <button type="button" onClick={() => setCreating(true)}>Registrar Riesgo</button>}</header>
    {error && <p className="form-error" role="alert">{error}</p>}
    <form className="record-filters module-filter-card module-filter-card--risks" onSubmit={applyFilters} aria-label="Filtros de Riesgos de trabajo">
    ...
    {loading ? <div className="module-state" role="status">Cargando Riesgos de trabajo…</div> : items.length === 0 ? <div className="module-empty-state"><span aria-hidden="true">⌁</span><div><strong>No existen Riesgos de trabajo.</strong><p>Pruebe ajustar los filtros o registre un nuevo riesgo cuando corresponda.</p></div></div> : <section className="module-table-card"><div className="module-table-card-heading"><div><h3>Registros de riesgo</h3><p>Información disponible según sus permisos actuales.</p></div><span className="badge">{total} registros</span></div><div className="table-scroll"><table className="data-table module-data-table"><thead><tr><th>Código</th><th>Fecha</th><th>Persona</th><th>Estado</th><th>Responsable</th><th>Actualización</th><th>Acciones</th></tr></thead><tbody>{items.map((item) => <tr key={item.id_caso}><td>{item.codigo_caso}<small>{item.id_caso}</small></td><td>{visible(item.fecha_apertura)}</td><td>{visible(item.persona)}<small>{visible(item.cedula)} · {visible(item.area)}</small><small>{visible(item.resumen)}</small></td><td><span className="badge">{item.estado_caso}</span></td><td>{visible(item.responsable)}<small>Por: {visible(item.registrado_por)}</small></td><td>{visible(item.fecha_actualizacion)}</td><td className="table-actions"><Link className="button-link secondary-link" to={`/trabajo-social/departamento-medico/riesgos/${item.id_caso}`}>Ver detalle</Link>{canEdit && item.estado_caso !== 'CERRADO' && <><button type="button" className="secondary" onClick={() => setEditing(item)}>Editar</button><button type="button" onClick={() => setClosing(item)}>Cerrar</button></>}</td></tr>)}</tbody></table></div></section>}
    <div className="pagination-controls module-pagination"><button type="button" className="secondary" disabled={!page || loading} onClick={() => setPage(page - 1)}>Anterior</button><span>{total ? `Mostrando ${page * pageSize + 1}-${Math.min(total, (page + 1) * pageSize)} de ${total}` : '0 registros'}</span><button type="button" className="secondary" disabled={loading || (page + 1) * pageSize >= total} onClick={() => setPage(page + 1)}>Siguiente</button></div>
