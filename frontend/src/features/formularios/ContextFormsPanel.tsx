...
import { listContextForms, listOfficeContextForms, listProductionContextForms, type ContextForm } from '../../api/formBuilder';
import { HttpError } from '../../api/client';
import { listarFormulariosRiesgo } from '../../api/riesgosTrabajo';
import { formatDateTime } from '../../utils/dates';

export function ContextFormsPanel({ contextType, contextId, riesgoId, produccionKind, oficinaKind }: { contextType: string; contextId: string; riesgoId?: string; produccionKind?: 'atenciones' | 'recorridos' | 'novedades'; oficinaKind?: 'atenciones' | 'beneficios' | 'prestamos' | 'seguro' }) {
  const [forms, setForms] = useState<ContextForm[]>([]); const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const load = useCallback(() => { setLoading(true); setError(null); (riesgoId ? listarFormulariosRiesgo(riesgoId) : produccionKind ? listProductionContextForms(produccionKind, contextId) : oficinaKind ? listOfficeContextForms(oficinaKind, contextId) : listContextForms(contextType, contextId)).then(setForms)
    .catch((err) => setError(err instanceof HttpError ? err.message : 'No fue posible cargar los formularios.')).finally(() => setLoading(false)); }, [contextType, contextId, riesgoId, produccionKind, oficinaKind]);
  useEffect(() => { load(); }, [load]);
  ...
  return <section className="panel context-forms-panel"><div className="panel-header"><div><h2>Formularios</h2><p className="footnote">Formularios publicados para este registro.</p></div><span className="badge">{forms.length} disponibles</span></div>
    ...
    {!forms.length && !error ? <div className="empty-state"><strong>Sin formularios disponibles</strong><p>No hay formularios publicados asignados a este módulo.</p></div> : <div className="context-form-list">{forms.map((form) => {
      ...
      const params = new URLSearchParams({ contexto_tipo: contextType, contexto_id: contextId }); if (riesgoId) params.set('riesgo_id', riesgoId); if (produccionKind) { params.set('produccion_tipo', produccionKind); params.set('produccion_id', contextId); } if (oficinaKind) { params.set('oficina_tipo', oficinaKind); params.set('oficina_id', contextId); }
      ...
      return <article key={form.id_formulario}><div><div className="context-form-title"><h3>{form.nombre}</h3><span className={`badge${answered ? ' badge--success' : draft ? ' badge--warning' : ''}`}>{answered ? 'RESPONDIDO' : form.estado_respuesta}</span></div>
        ...
        <div className="record-actions"><Link className="button-link" to={`/formularios/${form.id_formulario}/responder?${params}`}>{answered ? 'Ver respuesta' : draft ? 'Continuar' : 'Responder'}</Link>
          {answered && form.permite_multiples_respuestas && form.puede_crear && <Link className="button-link secondary-link" to={`/formularios/${form.id_formulario}/responder?${params}`}>Responder nuevamente</Link>}
        </div></article>;
    })}</div>}
  </section>;
}
